...
\begin{tikzpicture}[
    ...
    \node (checktime) [decision, below=0.8cm of lookup] {Trong khoảng $\pm 30$p giờ bắt đầu?};
    \node (refuse) [activity, right=1.5cm of checktime] {Từ chối check-in, thông báo ngoài khung giờ};
    \node (execute) [activity, below=0.8cm of checktime] {Cập nhật trạng thái \texttt{CHECKED\_IN}};
    ...
    \draw [arrow] (lookup) -- (checktime);
    \draw [arrow] (checktime) -- node[above] {Không} (refuse);
    \draw [arrow] (checktime) -- node[right] {Hợp lệ} (execute);
...
\begin{tikzpicture}[
    ...
    \node (checktime) [decision, below=0.8cm of query] {Thời gian tạo $> 15$ phút?};
    \node (skip) [activity, right=1.5cm of checktime] {Bỏ qua, chờ chu kỳ tiếp theo};
    \node (expire) [activity, below=0.8cm of checktime] {Cập nhật trạng thái thành \texttt{EXPIRED}};
    ...
    \draw [arrow] (query) -- (checktime);
    \draw [arrow] (checktime) -- node[above] {Chưa quá} (skip);
    \draw [arrow] (checktime) -- node[right] {Quá 15p} (expire);
...
\begin{tikzpicture}[
    ...
    \node (condrefund) [decision, below=0.8cm of policy] {Tỷ lệ hoàn tiền $> 0\%$?};
    \node (zero) [activity, right=1.5cm of condrefund] {Ghi nhận hủy, tiền hoàn bằng 0 VNĐ};
    \node (refund) [activity, below=0.8cm of condrefund] {Cộng tiền hoàn vào ví nội bộ của khách};
    ...
    \draw [arrow] (policy) -- (condrefund);
    \draw [arrow] (condrefund) -- node[above] {Bằng 0\%} (zero);
    \draw [arrow] (condrefund) -- node[right] {$> 0\%$} (refund);
